...
FSMStatePumpActive::FSMStatePumpActive(GameObject* pObj, DigDugGridComponent* pGrid, float offset, float duration, FSMState* pInactive,
	FSMState* pHit)
	: m_pObject(pObj)
	...
	if (m_pObject)
	{
		...
		if (pSprite)
		{
			...
			case Direction::Down:
				...
				extent.y += r.height * 0.5f * scale.y;
				break;
			case Direction::Up:
				t.SetLocalPosition(0, - (r.height*0.5f * scale.y + m_PlayerOffset + offset.y));
				extent.y -= r.height * 0.5f * scale.y;
			...
			//Check if not colliding => Return Inactive state
