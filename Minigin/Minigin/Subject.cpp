...
void Subject::Notify(const std::string& observername, GameObject* pGameObject, ObservedEvent event)
{
	if (std::find_if(m_pObservers.begin(), m_pObservers.end(), 
		[&observername]
	...
	{
		m_pObservers[observername]->Notify(pGameObject, event);
	}
	else
	{
...
void Subject::Notify(GameObject* pGameObject, ObservedEvent event)
{
	std::for_each(m_pObservers.begin(), m_pObservers.end(), 
		[pGameObject, event]
	(const std::pair<std::string, std::shared_ptr<Observer>>& p)
	{
		p.second->Notify(pGameObject, event);
	});
}
