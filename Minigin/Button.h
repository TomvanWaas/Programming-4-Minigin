...
namespace DigDug
{
	...
	class ButtonManager final : public Manager
	{
	public:
		void Next();
		void Previous();
	...
	public:
		...
		void SetStateNone(const std::shared_ptr<Font>& pFont, SDL_Color color) { m_pNoneFont = pFont; m_NoneColor = color; }
		void SetStateHover(const std::shared_ptr<Font>& pFont, SDL_Color color) { m_pHoverFont = pFont; m_HoverColor = color; }
	...
	private:
		...
		std::shared_ptr<Font> m_pNoneFont;
		SDL_Color m_NoneColor;
		std::shared_ptr<Font> m_pHoverFont;
